    ...
    def get_prob(self, query_vals, num_samples):
        ...
        for i in range(num_samples):
            ...
            for val in query_vals:
                if sample[val] == query_vals[val]:
                    val_match += 1
            if val_match == len(query_vals):
                count += 1
    ...
    def get_prob(self, query_vals, evidence_vals, num_samples):
        ...
        for i in range(num_samples):
            ...
            for val in evidence_vals:
                if sample[val] == evidence_vals[val]:
                    val_match += 1
            if val_match == (len(query_vals) + len(evidence_vals)):
                count += 1
        return count/num_samples
...
def bayes_sample_size_plot(sampler1, sampler2, query, evidence, label1, label2, title, fname):
    ...
    """
    x_vals = range(50, 10000, 20)
    y_vals1 = []
    y_vals2 = []
    for x in x_vals:
        y_vals1.append(sampler1.get_prob(query, evidence, x))
        y_vals2.append(sampler2.get_prob(query, evidence, x))
    two_line_plot(x_vals, y_vals1, label1, x_vals, y_vals2, label2, title, fname)
    return
...
if __name__ == '__main__':
    ...
    bayes_sample_size_plot(sampler_reject, sampler_like, 
                           ...
                           "C:\\Users\\Jasper\\OneDrive\\Documents\\CS383\\383_homework3\\bayes_awesome.pdf")
